...
def preprocessParseResults(parseResult):
    ...
    for i in range(len(parseResult['sentences'])):
        ...
        if i>0:
            for j in range(len(parseResult['sentences'][i]['dependencies'])):
                ...
                for k in range(1,3):
                    tokens = \
                        parseResult['sentences'][i]['dependencies'][j][k].split('-')
                    if tokens[0] == 'ROOT':
                        newWordIndex = 0
                    else:
    ...
    for i in range(len(parseResult['sentences'][0]['words'])):
        tag = \
         [[parseResult['sentences'][0]['words'][i][1]['CharacterOffsetBegin'],
          ...
          wordIndex, parseResult['sentences'][0]['words'][i][0],
        ...
        if nerWordAnnotations[i][3] == nerWordAnnotations[i-1][3] and \
           nerWordAnnotations[i][1] == nerWordAnnotations[i-1][1]+1:
            ...
            namedEntities.append([currentCharacterOffsets, currentWordOffsets,
                                  currentNE, nerWordAnnotations[i-1][3]])
