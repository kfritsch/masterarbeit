...
def suAlignmentScore(refAns, pupAns, question, simDf):
    global SIM_DF
    SIM_DF = simDf

    sentence1_parse_result = preprocessParseResults(refAns["alignAnn"])
    sentence1 = sentence1_parse_result["sentences"][0]["text"]
    sentence2_parse_result = preprocessParseResults(pupAns["alignAnn"])
    sentence2 = sentence2_parse_result["sentences"][0]["text"]
    sentence_for_demoting_parse_result = question["alignAnn"]
    parse_results = [sentence1_parse_result, sentence2_parse_result, sentence_for_demoting_parse_result]
...
def alignNouns(source, target, sourceParseResult, targetParseResult, existingAlignments):
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            if max(wordRelatedness(sourceWords[i-1], sourcePosTags[i-1], targetWords[j-1], targetPosTags[j-1]), wordRelatedness(sourceLemmas[i-1], sourcePosTags[i-1], targetLemmas[j-1], targetPosTags[j-1]))<SIM_THRES:
                continue
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordParents:
                    if ((ktem[0], ltem[0]) in existingAlignments+nounAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=SIM_THRES) and (
                        (ktem[2]==ltem[2]) or
    ...
    for n in range(numberOfNounsInSource):
        ...
        for i in sourceWordIndices:
            ...
            for j in targetWordIndices:
                ...
                if (i, j) in evidenceCountsMatrix and THETA*wordSimilarities[(i, j)]+(1-THETA)*evidenceCountsMatrix[(i, j)]>maxOverallValueForCurrentPass:
                    maxOverallValueForCurrentPass = THETA*wordSimilarities[(i, j)]+(1-THETA)*evidenceCountsMatrix[(i, j)]
...
def alignMainVerbs(source, target, sourceParseResult, targetParseResult, existingAlignments):
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            for ktem in sourceWordChildren:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in existingAlignments+mainVerbAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=SIM_THRES) and (
                            (ktem[2]==ltem[2]) or
...
def alignAdjectives(source, target, sourceParseResult, targetParseResult, existingAlignments):
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            for ktem in sourceWordChildren:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in existingAlignments+adjectiveAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=SIM_THRES) and (ktem[2]==ltem[2]):
                        if (i, j) in evidenceCountsMatrix:
                            evidenceCountsMatrix[(i, j)] += max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
                        else:
                            evidenceCountsMatrix[(i, j)] = max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in existingAlignments+adjectiveAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=SIM_THRES) and (
                            (ktem[2]==ltem[2]) or
...
def alignAdverbs(source, target, sourceParseResult, targetParseResult, existingAlignments):
    ...
    for i in sourceWordIndices:
        ...
        for j in targetWordIndices:
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordParents:
                    if ((ktem[0], ltem[0]) in existingAlignments+adverbAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=SIM_THRES) and (ktem[2]==ltem[2]):
                        if (i, j) in evidenceCountsMatrix:
                            evidenceCountsMatrix[(i, j)] += max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
                        else:
                            evidenceCountsMatrix[(i, j)] = max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))
            ...
            for ktem in sourceWordParents:
                for ltem in targetWordChildren:
                    if ((ktem[0], ltem[0]) in existingAlignments+adverbAlignments or max(wordRelatedness(ktem[1], sourcePosTags[ktem[0]-1], ltem[1], targetPosTags[ltem[0]-1]), wordRelatedness(sourceLemmas[ktem[0]-1], sourcePosTags[ktem[0]-1], targetLemmas[ltem[0]-1], targetPosTags[ltem[0]-1]))>=SIM_THRES) and (
                            (ktem[2]==ltem[2]) or
...
def alignWords(source, target, sourceParseResult, targetParseResult):
    ...
    sequenceAlignments = []
    namedEntityAlignments = []
    dependencyNeighborhoodAlignments = []
    textualNeighborhoodAlignments = []
    noContextualOverlapAlignments = []
    ...
    if (sourceWords[len(source)-1] in ['.', '!'] and targetWords[len(target)-1] in ['.', '!']) or sourceWords[len(source)-1]==targetWords[len(target)-1]:
        ...
        noContextualOverlapAlignments.append([len(source), len(target)])
        ...
        noContextualOverlapAlignments.append([len(source)-1, len(target)])
        ...
        noContextualOverlapAlignments.append([len(source), len(target)-1])
        sourceWordIndicesAlreadyAligned.append(len(source))
        ...
        noContextualOverlapAlignments.append([len(source)-1, len(target)-1])
        sourceWordIndicesAlreadyAligned.append(len(source)-1)
        targetWordIndicesAlreadyAligned.append(len(target)-1)
    ...
    for item in commonContiguousSublists:
        allstop_words = True
        for jtem in item:
            if jtem not in stop_words and jtem not in punctuations:
                allstop_words = False
                break
        if len(item[0]) >= 2 and not allstop_words:
                ...
                if len(item[0]) > 1:
                    for jtem in item[1]:
                        if [i, jtem+1] not in alignments:
                            ...
                            sequenceAlignments.append([i, jtem+1])
                ...
                if len(item[0]) > 1:
                    for jtem in item[0]:
                        if [jtem+1, i] not in alignments:
                            ...
                            sequenceAlignments.append([jtem+1, i])
    ...
    for item in neAlignments:
        if item not in alignments:
            ...
            namedEntityAlignments.append(item)
    ...
    for item in mainVerbAlignments:
        if item not in alignments:
            alignments.append(item)
            dependencyNeighborhoodAlignments.append(item)
            if item[0] not in sourceWordIndicesAlreadyAligned:
                sourceWordIndicesAlreadyAligned.append(item[0])
            if item[1] not in targetWordIndicesAlreadyAligned:
                targetWordIndicesAlreadyAligned.append(item[1])
    ...
    for i in sourceWordIndices:
        if i in sourceWordIndicesAlreadyAligned or sourceLemmas[i-1] in stop_words + punctuations + ['\'s', '\'d', '\'ll']:
            continue

        for j in targetWordIndices:
            if j in targetWordIndicesAlreadyAligned or targetLemmas[j-1] in stop_words + punctuations + ['\'s', '\'d', '\'ll']:
                continue
            ...
            for k in range(len(sourceNeighborhood[0])):
                for l in range(len(targetNeighborhood[0])):
                      if (sourceNeighborhood[1][k] not in stop_words + punctuations) and ((sourceNeighborhood[0][k], targetNeighborhood[0][l]) in alignments or (wordRelatedness(sourceNeighborhood[1][k], 'none', targetNeighborhood[1][l], 'none')>=SIM_THRES)):
    ...
    for item in range(numOfUnalignedWordsInSource):
        ...
        for i in sourceWordIndicesBeingConsidered:
            ...
            for j in targetWordIndicesBeingConsidered:
                ...
                theta2 = 1 - THETA
                if THETA*wordSimilarities[(i, j)] + theta2*textualNeighborhoodSimilarities[(i, j)] > highestWeightedSim:
                    highestWeightedSim = THETA*wordSimilarities[(i, j)] + theta2*textualNeighborhoodSimilarities[(i, j)]
                    bestSourceIndex = i
                    bestTargetIndex = j
                    bestWordSim = wordSimilarities[(i, j)]
                    bestTextNeighborhoodSim = textualNeighborhoodSimilarities[(i, j)]

        if bestWordSim>=SIM_THRES and [bestSourceIndex, bestTargetIndex] not in alignments:
            if sourceLemmas[bestSourceIndex-1] not in stop_words:
                alignments.append([bestSourceIndex, bestTargetIndex])
                if bestTextNeighborhoodSim > 0:
                    textualNeighborhoodAlignments.append([bestSourceIndex, bestTargetIndex])
                else:
                    noContextualOverlapAlignments.append([bestSourceIndex, bestTargetIndex])
    ...
    for i in sourceWordIndices:
        ...
        if '-' in sourceWords[i-1] and sourceWords[i-1] != '-':
            ...
            commonContiguousSublists = findAllCommonContiguousSublists(tokens, targetWords)
            for item in commonContiguousSublists:
                if len(item[0]) == 1 and target[item[1][0]][3] not in stop_words:
                    for jtem in item[1]:
                        if [i, jtem+1] not in alignments and jtem+1 not in targetWordIndicesAlreadyAligned:
                            alignments.append([i, jtem+1])
                            noContextualOverlapAlignments.append([i, jtem+1])
                            sourceWordIndicesAlreadyAligned.append(i)
                            targetWordIndicesAlreadyAligned.append(jtem+1)
    ...
    for i in targetWordIndices:
        ...
        if '-' in target[i-1][2] and target[i-1][2] != '-':
            ...
            commonContiguousSublists = findAllCommonContiguousSublists(sourceWords, tokens)
            for item in commonContiguousSublists:
                if len(item[0]) == 1 and source[item[0][0]][3] not in stop_words:
                    for jtem in item[0]:
                        if [jtem+1, i] not in alignments and i not in targetWordIndicesAlreadyAligned:
                            alignments.append([jtem+1, i])
                            noContextualOverlapAlignments.append([jtem+1, i])
                            sourceWordIndicesAlreadyAligned.append(jtem+1)
                            targetWordIndicesAlreadyAligned.append(i)
    ...
    '''
    # collect evidence from dependency neighborhood for aligning stop_words
    wordSimilarities = {}
    ...
    for i in sourceWordIndices:
        if sourceLemmas[i-1] not in stop_words or i in sourceWordIndicesAlreadyAligned:
            continue

        for j in targetWordIndices:
            if targetLemmas[j-1] not in stop_words or j in targetWordIndicesAlreadyAligned:
                continue

            if (sourceLemmas[i-1]!=targetLemmas[j-1]) and (wordRelatedness(sourceLemmas[i-1], sourcePosTags[i-1], targetLemmas[j-1], targetPosTags[j-1])<SIM_THRES):
                continue
        ...
        for i in sourceWordIndicesBeingConsidered:
            for j in targetWordIndicesBeingConsidered:
                if (i, j) not in wordSimilarities:
                    continue
                theta2 = 1 - THETA
                if THETA*wordSimilarities[(i, j)] + theta2*dependencyNeighborhoodSimilarities[(i, j)] > highestWeightedSim:
                    highestWeightedSim = THETA*wordSimilarities[(i, j)] + theta2*dependencyNeighborhoodSimilarities[(i, j)]
                    bestSourceIndex = i
                    bestTargetIndex = j
                    bestWordSim = wordSimilarities[(i, j)]
                    bestDependencyNeighborhoodSim = dependencyNeighborhoodSimilarities[(i, j)]

        if bestWordSim>=SIM_THRES and bestDependencyNeighborhoodSim>0 and [bestSourceIndex, bestTargetIndex] not in alignments:
            alignments.append([bestSourceIndex, bestTargetIndex])
            dependencyNeighborhoodAlignments.append([bestSourceIndex, bestTargetIndex])
            sourceWordIndicesAlreadyAligned.append(bestSourceIndex)
            targetWordIndicesAlreadyAligned.append(bestTargetIndex)
    ...
    # collect evidence from textual neighborhood for aligning stop_words and punctuations
    wordSimilarities = {}
    textualNeighborhoodSimilarities = {}
    sourceWordIndicesBeingConsidered = []
    targetWordIndicesBeingConsidered = []
    ...
    for i in sourceWordIndices:
        if (sourceLemmas[i-1] not in stop_words + punctuations + ['\'s', '\'d', '\'ll']) or i in sourceWordIndicesAlreadyAligned:
            continue

        for j in targetWordIndices:
            if (targetLemmas[j-1] not in stop_words + punctuations + ['\'s', '\'d', '\'ll']) or j in targetWordIndicesAlreadyAligned:
                continue

            if wordRelatedness(sourceLemmas[i-1], sourcePosTags[i-1], targetLemmas[j-1], targetPosTags[j-1]) < SIM_THRES:
                continue
    ...
    for item in range(numOfUnalignedWordsInSource):
        ...
        if bestWordSim>=SIM_THRES and bestTextNeighborhoodSim>0 and [bestSourceIndex, bestTargetIndex] not in alignments:
            alignments.append([bestSourceIndex, bestTargetIndex])
            textualNeighborhoodAlignments.append([bestSourceIndex, bestTargetIndex])
            sourceWordIndicesAlreadyAligned.append(bestSourceIndex)
            targetWordIndicesAlreadyAligned.append(bestTargetIndex)
    ...
    sequenceAlignments = [item for item in sequenceAlignments if item[0]!=0 and item[1]!=0]
    namedEntityAlignments = [item for item in namedEntityAlignments if item[0]!=0 and item[1]!=0]
    dependencyNeighborhoodAlignments = [item for item in dependencyNeighborhoodAlignments if item[0]!=0 and item[1]!=0]
    textualNeighborhoodAlignments = [item for item in textualNeighborhoodAlignments if item[0]!=0 and item[1]!=0]
    noContextualOverlapAlignments = [item for item in noContextualOverlapAlignments if item[0]!=0 and item[1]!=0]


    return [alignments, sequenceAlignments, namedEntityAlignments, dependencyNeighborhoodAlignments, textualNeighborhoodAlignments, noContextualOverlapAlignments]
...
def align(sentence1, sentence2,
    ...
    myWordAlignments = alignWords(sentence1LemmasAndPosTags,
                                  sentence2LemmasAndPosTags,
...
def getCanonicalWord(word):
    if len(word) > 1:
        canonicalWord = word.replace('.', '')
        canonicalWord = canonicalWord.replace('-', '')
        canonicalWord = canonicalWord.replace(',', '')
    else:
        canonicalWord = word
...
def wordRelatedness(word1, pos1, word2, pos2):
    canonicalWord1 = getCanonicalWord(word1)
    canonicalWord2 = getCanonicalWord(word2)

    if canonicalWord1.lower() == canonicalWord2.lower():
        return 1
    elif(canonicalWord1 in SIM_DF.columns and canonicalWord2 in SIM_DF.index):
        sim = SIM_DF.loc[canonicalWord2,canonicalWord1]
        if(sim != np.nan):
            return sim
        else:
            return 0
